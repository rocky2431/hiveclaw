...
|---|---|---:|---|---|---|---|
| TRANSCRIPT-PROJECTION-TERMINAL-DEADLETTER-001 | Verified / fresh-run consumption pending | P1 | P01-MAIN / P02-STREAM | completed employee run产生1033条transcript events；约12分钟后仍有118条pending，5秒sweeper约每轮只推进一个frontier；required terminal outbox在attempt 8先以`WebTerminalBoundaryPending`进入dead letter，RuntimeTask `completion_outbox_settled_at`保持null | 共享有序排空修正经zCode、CC `4fc7c994…`与Codex红/绿接受，源码/test SHA-256=`bb13510c…`/`d181503a…`，RLS `94a52136…`以584摘要项/109签名零授权delta登记；exact `53e23d1a` Harness `34037372686`三job全绿并三服务同源部署。production 1033条已全projected，证明frontier已排空；旧dead letter仍需恢复且暴露下方独立idle-seal根因 | 保留共享修正；fresh P01需证明新terminal不再因大prefix dead-letter后才可Closed，不把下方idle-seal失败回退为本修正未部署 |
| T0-IDLE-SEAL-TERMINAL-RECOVERY-001 | Fix candidate / local review accepted | P1 | P01-MAIN / P02-STREAM | exact `53e23d1a`部署后对`7b200f1c…`唯一一次operator redrive为HTTP 200/audit `df70806a…`，worker仍在attempt9以`WebTerminalBoundaryPending`dead-letter，未delivery且RuntimeTask未settle | DB terminal seq1033映射T0 `evt_ee53…`/`seg-20260906T123523Z-dfd42344`；支持的files API只读证明该段由`SESSION_IDLE / idle_timeout`封成seq1034且段及seq1032–1034只有exact RuntimeTask、没有`turn_id`。首候选及stored-turn中间态均被否定；生产形态候选又被CC `51c3cf03…`与Codex阻断alias死代码及真实required-T2测试缺口。zCode `ff19df81…`最终删除alias并补真实projector held/failed回归，四文件冻结hash=`6782fae6…`/`04c4a44a…`/`3067d292…`/`895e8c17…`；CC `bdb19fa5…`独立接受。Codex独立复核候选67、旧码7红、邻接59+278、真实PG 3、RLS 17及Ruff/format/diff-check全绿，确认adoption只读且不削弱failed gate | 仅提交本轮四份backend文件与文档/evidence，等待exact CI三job全绿后从clean archive三服务同源部署；部署前后只读确认T2仍`held`，再只做一次attempt10并核对单audit/delivery/settlement和零业务重复；当前不得redrive |
